...
-----------------------------------------------------
〇MeshBoundsSetter(ver1.02) 【サポート終了（VRCAvatarEditorに同様の機能を実装済み）】
...
-----------------------------------------------------
〇ProbeAnchorSetter(ver1.0) 【サポート終了（VRCAvatarEditorに同様の機能を実装済み）】
...
〇VRCAssetCreator (ver1.0)
VRChatに関連したAssetを右クリックで作成できるようにします
現在はVRChatアバター用のAnimatorOverrideControllerの作成ができます

ProjectタブのAssetを作成したいフォルダで右クリック>Create>VRChat>...でそれぞれのAssetを右クリックしたフォルダに作成します

・更新履歴
ver1.0 VRChatアバター用のAnimatorOverrideControllerを作成可能に

-----------------------------------------------------

〇ShapeKeyDeleter(ver1.0)
メッシュのシェイプキーを削除します
Meshを複製して新しいMeshを作成してシェイプキーを削除しているので元データに戻すことは可能です。
@vjroba からShapeKeyMixerを改造したものとしていただいてさらに改良したものです
...
1. 「Renderer」に合成したいシェイプキーを持つメッシュのオブジェクトを設定します。
（SkinnedMeshRendererコンポーネントを持つオブジェクト）
2. 削除したいシェイプキーにチェックをいれます。
3. 合成したいシェイプキーを全て選択したら「Delete ShapeKeys」を選択します。
4. 3の後にシェイプキーの削除をなかったことにしたい場合は変更後にCtrl+Zで変更前のシェイプキーの状態に戻ります。
